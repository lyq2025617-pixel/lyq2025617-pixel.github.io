...
function buildPrompt(inputs) {
  ...
  const venueStr   = venue  ? `活动场地：${venue}，` : '';
  const dateStr    = date   ? `活动日期：${date}，` : '';
  const durationLabel = { '60':'1小时','120':'2小时','180':'3小时','240':'半天' };
  const durationText  = durationLabel[String(duration)] || `${duration}分钟`;
  const toneMap = { 
    '活泼':'语气轻松热情，充满活力，像朋友聊天一样亲切自然，使用感叹号和表情符号增加感染力', 
    '搞笑':'幽默风趣，适当使用网络流行语和梗，让现场充满笑声，语言夸张有趣', 
    '正式':'措辞专业庄重，有仪式感，适合重要场合，使用敬语和正式表达', 
    '专业':'措辞专业庄重，有仪式感，逻辑清晰，内容详实', 
    '温馨':'语气温柔亲切，充满情感和温暖，让听众感受到关怀', 
    '温情':'语气温柔亲切，充满情感和温暖，适合表达感恩和祝福',
    '燃':'激情澎湃，节奏感强，使用有力的词汇，点燃全场热情', 
    '治愈':'语气温和舒缓，让人放松心情，感受到宁静与美好'
  };
  const primaryStyle = (styles || []).find(s => toneMap[s]);
  const scriptTone   = primaryStyle ? toneMap[primaryStyle] : `符合「${stylesStr}」风格`;

  return `你是一位经验丰富的专业校园活动策划师，擅长策划各种类型的校园活动。请根据以下详细信息，为这场活动生成一份完整、丰富、专业的策划方案，并以 JSON 格式返回。

【活动基本信息】
- 活动主题：${theme}
- 活动类型：${type}（如晚会、迎新、毕业、联谊、社团招新等）
- 参与人数：${people}人
- 活动时长：${durationText}
- 活动风格：${stylesStr}
- ${venueStr}${dateStr}
...
【输出格式要求】
请严格按照以下 JSON 结构输出（仅输出纯 JSON，不要包含任何其他文字或 markdown 代码块）：
{
  "process": [
    { 
      "time": "开始时间-结束时间（格式如 18:30-18:45）", 
      "title": "环节名称", 
      "desc": "环节详细说明，包括活动内容、参与方式、预期效果等，100-150字" 
    }
  ],
  "playlist": [
    {
      "section": "歌单分区名（如 开场预热、互动高潮、温情时刻）",
      "songs": [
        { "title": "歌曲名称", "artist": "歌手/乐队", "duration": "时长（如 3:42）", "cover": "🎵", "mood_tags": ["情绪标签1","情绪标签2","情绪标签3"] }
      ]
    }
  ],
  "script": [
    { "time": "时间段（如 18:30-18:45 开场环节）", "content": "主持人串词，150-200字，包含问候、环节介绍、互动引导等" }
  ],
  "tasks": [
    { "title": "任务名称", "desc": "任务详细说明，包括具体操作步骤和注意事项，50-80字", "done": false }
  ]
}
...
1. 【活动流程 process】
   - 生成6-8个环节，时间从18:30开始，总时长严格等于${durationText}
   - 每个环节要有明确的主题和丰富的内容描述
   - 环节之间要有逻辑衔接，形成完整的活动流程
   - 包含开场、主体活动、互动环节、收尾等部分
   - 描述要具体，说明每个环节的活动形式、参与方式和预期效果

2. 【专属歌单 playlist】
   - 生成3-4个歌单分区，每个分区3-4首歌曲
   - 歌曲必须是真实存在的流行歌曲，符合活动风格「${stylesStr}」
   - 每个分区要有明确的主题（如开场预热、互动高潮、温情时刻、结束送别）
   - mood_tags 要准确描述歌曲的情绪和适合的场景

3. 【主持串词 script】
   - 每个环节对应一条串词，语气要求：${scriptTone}
   - 串词要生动有趣，包含问候语、环节介绍、互动引导、过渡语等
   - 适当加入互动提问，引导观众参与
   - 结尾要有感谢和结束语

4. 【任务清单 tasks】
   - 生成8-10条筹备任务
   - 任务要全面覆盖活动筹备的各个方面（场地、宣传、物资、人员、节目、应急预案等）
   - 每个任务要有详细的说明，包括具体操作步骤
   - 合理设置已完成状态（done: true/false）

5. 【整体要求】
   - 所有内容必须紧密围绕「${theme}」主题展开
   - 充分体现「${stylesStr}」风格特点
   - 语言要生动、专业、富有感染力
   - 确保 JSON 格式正确，无语法错误
   - 严格输出纯 JSON，禁止输出任何解释性文字`;
}
